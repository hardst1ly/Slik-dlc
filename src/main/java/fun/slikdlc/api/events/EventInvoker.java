...
public class EventInvoker {
   ...
   private static final ConcurrentHashMap<Class<? extends Event>, List<EventInvoker.Invocation>> invocationCache = new ConcurrentHashMap<>();
   ...
   private static final long SLOW_HANDLER_NANOS = Long.getLong("slikdlc.perf.handlerMs", 8L) * 1000000L;
   private static final long SLOW_EVENT_NANOS = Long.getLong("slikdlc.perf.eventMs", 18L) * 1000000L;
   private static final long WARN_COOLDOWN_NANOS = Long.getLong("slikdlc.perf.cooldownMs", 1000L) * 1000000L;
   ...
   public static void invoke(Event event) throws IllegalAccessException, InvocationTargetException, InstantiationException {
      ...
      List<EventInvoker.Invocation> invocations = invocationCache.get(event.getClass());
      if (invocations != null && !invocations.isEmpty()) {
         for (EventInvoker.Invocation invocation : invocations) {
   ...
   public static boolean hasListeners(Class<? extends Event> eventClass) {
      ...
      List<EventInvoker.Invocation> invocations = invocationCache.get(eventClass);
      return invocations != null && !invocations.isEmpty();
   }

   private static synchronized void rebuildCache() {
      if (cacheDirty) {
         ConcurrentHashMap<Class<? extends Event>, List<EventInvoker.Invocation>> rebuilt = new ConcurrentHashMap<>();
               ...
               if (method.isAnnotationPresent(EventLink.class)) {
                  ...
                  if (parameters.length == 1 && Event.class.isAssignableFrom(parameters[0])) {
                     ...
                     rebuilt.computeIfAbsent(eventClass, key -> new ArrayList<>())
                        .add(new EventInvoker.Invocation(listener, method, method.getAnnotation(EventLink.class).priority()));
                  }
               }
            }
         }

         for (List<EventInvoker.Invocation> invocations : rebuilt.values()) {
   ...
   private static void logSlowHandler(Event event, EventInvoker.Invocation invocation, long elapsedNanos) {
      ...
      if (canWarn(slowHandlerWarnings, key)) {
         System.out
            .println(
               String.format(Locale.ROOT, "[PerfDebug] Slow handler: %s -> %s#%s took %.2f ms", eventName, listenerName, methodName, elapsedNanos / 1000000.0)
            );
      }
   }
   ...
   private static void logSlowEvent(Event event, long elapsedNanos, int invocationCount) {
      ...
      if (canWarn(slowEventWarnings, key)) {
         System.out
            .println(
               String.format(Locale.ROOT, "[PerfDebug] Slow event: %s took %.2f ms for %d handlers", eventName, elapsedNanos / 1000000.0, invocationCount)
            );
      }
   }
